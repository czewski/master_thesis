...
def get_hit(indices, targets):
    ...
    Parameters:
    - indices (torch.Tensor): The top K indices predicted by the model.
                              Shape: (num_users, K)
    - targets (torch.Tensor): The ground truth indices of the items.
                              Shape: (num_users,)

    Returns:
    - hit (float): The Hit@K score.
    """
    hits = (indices == targets.view(-1, 1)).any(dim=1).float()
    hit = hits.mean().item()
    return hit
